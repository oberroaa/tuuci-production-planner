        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 space-y-5 animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 pr-1 space-y-3" id="printable-qr-tickets">
              ...
              {/* Grid of thermal barcode tickets (Formatted for ULINE S-6263 4x3" Direct Thermal Labels) */}
              <div className="grid grid-cols-1 gap-3 pt-1">
                {(reprintModalJob.piecesList || []).map((pieceCode: string, idx: number) => {
                  const totalPiezas = reprintModalJob.piecesList?.length || reprintModalJob.cantidad_piezas || 1;
                  const itemCode = reprintModalJob.item_code || reprintModalJob.itemCode || 'OM7.5SQ';
                  const modeloDesc = reprintModalJob.modelo || 'OceanMaster M1 Classic 7.5\' SQ';
                  const jobCode = reprintModalJob.job_code || reprintModalJob.jobCode || pieceCode.split('-')[0];
                  
                  // Specs lines breakdown
                  const jobConfig = reprintModalJob.config;
                  ...
                  if (specLines.length === 0) {
                    const rawSpecs = reprintModalJob.specs_raw || reprintModalJob.specsRaw || '';
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Bottom Barcode 128 (Large, clear, industrial scanner ready) */}
